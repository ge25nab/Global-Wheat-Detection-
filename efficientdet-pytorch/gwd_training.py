...
def get_train_transforms():
    return A.Compose(
        [
            ...
            A.OneOf([A.Cutout(num_holes=8, max_h_size=64, max_w_size=64, fill_value=0, p=0.5),
                    A.Blur(blur_limit=5,p=0.5)],p=0.5),
...
class DatasetRetriever(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.test or random.random() > 0.8:
...
class Fitter:
    ...
    def fit(self, train_loader, validation_loader):
        for e in range(self.config.n_epochs):
            ...
            self.save(f'{self.base_dir}/last-fold{str(fold_num)}-checkpoint.bin')
            ...
            if summary_loss.avg < self.best_summary_loss:
                ...
                self.model.eval()
                self.save(f'{self.base_dir}/best-checkpoint-fold{str(fold_num)}-{str(self.epoch).zfill(3)}epoch.bin')
